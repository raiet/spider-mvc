...
class Dict(dict):
	...
	##这里的zip返回一个tuple，同时他会根据参数序列中最短的一个截断
	##比如zip（（a,b,c）,(1,2,3,4)）,则他之返回(a,1),(b,2),(c,3),并不会返回4
...
def next_id(t=None):
    ...
    if t is None:
        t = time.time()
    #使用UUID库生成唯一ID，UUID它通过MAC地址、时间戳、命名空间、随机数、伪随机数来保证生成ID的唯一性。uuid4()是使用伪随机数得到，有一定的重复概率
    ##uuid生成一个128位的字符串，十六进制表示就是32个字节，补充三个0，外加15位的数字，就是50是个字节
...
def create_engine(user, password, database, host='127.0.0.1', port=3306, **kw):
	...
	#If key is not found, d is returned if given, otherwise KeyError is raised
        params[k] = kw.pop(k, v)
    ##this is followed by: for k in kw: params[k] = kw[k]
    params.update(kw)
    ##创建连接
    engine = _Engine(lambda: MySQLdb.connect(**params))
    # test connection...
    logging.info('Init mysql engine <%s> ok.' % hex(id(engine)))
##定义with语句，将数据库的打开和关闭的操作交给这里处理，程序员只需要西自己的业务逻辑
...
def delete_all():
    update(r"drop  table user")

if __name__=='__main__':
    logging.basicConfig(level=logging.DEBUG)
    create_engine('root', 'csoffer607hello', 'test','112.124.41.235')
